...
if card.area and card.area == G.pack_cards and card.ability.blind_card then
  ...
  if card.area and card.area.config.type == 'joker' then
      if card.config.center.key == "j_bunc_stylophone" or card.config.center.key == "j_bunc_mousetrap" or card.config.center.key == "j_bunc_trigger_finger" then
          local settings_sprite = Sprite(0, 0, 0.5, 0.5, G.ASSET_ATLAS['bunc_settings_icon'] ,{x=0, y=0})
          use = {n=G.UIT.C, config={align = "cr"}, nodes={
              {n=G.UIT.C, config={ref_table = card, align = "cr",maxw = 1.25, padding = 0.1, r=0.08, minh = (card.area and card.area.config.type == 'joker') and 0 or 1, hover = true, shadow = true, colour = G.C.UI.BACKGROUND_INACTIVE, button = card.config.center.key .. '_settings'}, nodes={
                  {n=G.UIT.B, config = {w=0.1,h=0.6}},
                  {n=G.UIT.O, config={object = settings_sprite, can_collide = false, shadow = true}}
              }}
          }}
      end
...
function G.UIDEF.card_focus_ui(card)
  ...
  card.ability.consumeable and card.ability.set ~= 'csau_Stand' then --Add a use button
...
function G.UIDEF.card_focus_button(args)
  ...
  elseif args.type == 'use' then
    button_contents = {n=G.UIT.T, config={text = args.card.ability.set == 'VHS' and localize('b_'..(args.card.ability.activated and 'de' or '')..'activate') or localize('b_use'),colour = G.C.WHITE, scale = 0.5}}
        ...
        for k, v in ipairs(G.GAME.tags) do
          ...
          for _, v in ipairs(rates) do
            if polled_rate > check_rate and polled_rate <= check_rate + v.val then
              local card = create_card(v.type, area, nil, nil, nil, nil, (v.type == 'Joker') and (G.GAME.modifiers.forced_joker or G.GAME.modifiers.forced_joker_all or pseudorandom_element(G.GAME.modifiers.forced_joker_pool)) or nil, 'sho')
...
{n=G.UIT.R, config={align = "cm", minw = 1.5, minh = 0.4, r = 0.1, padding = 0.05, colour = desc_nodes.background_colour or G.C.WHITE}, nodes={{n=G.UIT.R, config={align = "cm", padding = 0.03}, nodes=t},d6_side_UI_config,}}
  ...
  function G.UIDEF.card_h_popup(card)
    if card.ability_UIBox_table then
      ...
      card_type = ((card.ability and card.ability.dsix_infected) and AUT.card_type ~= 'Enhanced') and localize('k_dsix_infected') or (debuffed and AUT.card_type ~= 'Enhanced') and localize('k_debuffed') or card_type
      ...
      if AUT.badges then
        for k, v in ipairs(AUT.badges) do
          ...
          if v:sub(1, 24) == 'bunc_consumable_edition_' then v = v:sub(25) end
          if v == "holo" then v = "holographic" end
  ...
  function get_badge_colour(key)
    G.BADGE_COL = G.BADGE_COL or {
      ...
      st_cruel_overpriced = G.C.ORANGE,
      st_cruel_wash = G.C.BLUE,
      puzzled = G.C.PERISHABLE,
    }
...
function create_UIBox_blind_select()
  ...
  G.GAME.blind_on_deck = 
    ...
    not (G.GAME.round_resets.blind_states.ChDp_Boss2 == 'Defeated' or G.GAME.round_resets.blind_states.ChDp_Boss2 == 'Skipped'or G.GAME.round_resets.blind_states.ChDp_Boss2 == 'Hide') and 'ChDp_Boss2' or 
  ...
  local t = {n=G.UIT.ROOT, config = {align = 'tm',minw = width, r = 0.15, colour = G.C.CLEAR}, nodes={
    {n=G.UIT.R, config={align = 'cm', padding = (G.GAME.modifiers.chdp_third_boss or G.GAME.modifiers.second_boss) and 0.2 or 0.5}, nodes= G.GAME.modifiers.chdp_third_boss and {
...
function create_UIBox_blind_tag(blind_choice, run_info)
  ...
  if G.GAME.modifiers.disable_skipping or not G.GAME.round_resets.blind_tags[blind_choice] then return nil end
  ...
  {n=G.UIT.R, config={id = 'tag_container', ref_table = _tag, align = "cm"}, nodes={
    {n=G.UIT.R, config={align = 'tm', minh = 0.65}, nodes={
      ...
      G.GAME.modifiers.chaos_engine_skip and {n=G.UIT.R, config={align = 'tm'}, nodes={
...
function create_UIBox_blind_choice(type, run_info)
  ...
  {n=G.UIT.R, config={id = type, align = "tm", func = 'blind_choice_handler', minh = not run_info and 10 or nil, ref_table = {deck = nil, run_info = run_info}, r = 0.1, padding = 0.05}, nodes={
    (G.GAME.modifiers.second_boss and G.GAME.round_resets.blind_states.Big ~= 'Hide' and G.GAME.round_resets.blind_states.Small ~= 'Hide') and {n=G.UIT.R, config={align = 'cm', colour = mix_colours(G.C.BLACK, G.C.L_BLACK, 0.5), r = 0.1, outline = 1, outline_colour = G.C.L_BLACK}, nodes={  
...
function create_UIBox_customize_deck()
  ...
  for i, suit in ipairs(SMODS.Suit:obj_list(true)) do
    ...
    if G.COLLABS.options[suit.key] and
...
function create_progress_box(_profile_progress, smaller)
  local rows, protos = {}, {'collection', 'challenges', 'joker_stickers', 'stand_stickers', 'deck_stake_wins'}
...
function G.UIDEF.stake_description(_stake)
  ...
  local ret_nodes = {}
  local target = {}
  if _stake_center and type(_stake_center.loc_vars) == "function" then
      res = _stake_center:loc_vars() or {}
      target.vars = res.vars or target.vars
      target.key = res.key or target.key
      target.set = res.set or target.set
  end
  if _stake_center then localize{type = 'descriptions', key = target.key or _stake_center.key, set = target.set or _stake_center.set, nodes = ret_nodes, vars = target.vars} end
...
function G.UIDEF.challenge_list_page(_page)
  ...
  for k, v in ipairs(G.CHALLENGES) do
    if (k > G.CHALLENGE_PAGE_SIZE*(_page or 0) and k <= G.CHALLENGE_PAGE_SIZE*((_page or 0) + 1)) and not v.hidden then
...
function G.UIDEF.challenge_description(_id, daily, is_row, is_hardcore)
  ...
  if challenge.jokers  then 
    for k, v in ipairs(challenge.jokers) do
      ...
      if v.singular then card.ability.chdp_singular = true end
      if v.shrouded then card.ability.chdp_shrouded = true end
      if v.pinned then card.pinned = true end
      if v.mxms_posted then card.ability.mxms_posted = true end
      if v.gemslot then Gemstones.set_gemslot(card, v.gemslot) end
