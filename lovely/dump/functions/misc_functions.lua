...
function get_blind_main_colour(blind) --either in the form of the blind key for the P_BLINDS table or type
  ...
  blind = blind or ''
  if blind == 'Boss' or blind == 'Small' or blind == 'Big' or blind == 'ChDp_Boss' or blind == 'ChDp_Boss2' then
...
function set_consumeable_usage(card)
    if card.config.center_key and card.ability.consumeable then
      ...
      if skill_active("sk_cry_ace_2") and (card.config.center.set == 'Code') then 
              G.E_MANAGER:add_event(Event({
                trigger = 'immediate',
                func = function()
                  G.E_MANAGER:add_event(Event({
...
function set_discover_tallies()
  ...
  end  for _, v in pairs(G.P_BLINDS) do
