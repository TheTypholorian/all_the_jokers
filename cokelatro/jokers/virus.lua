SMODS.Joker{ --Virus
    ...
    loc_txt = {
        ['name'] = 'Virus',
        ['text'] = {
            [1] = '{C:dark_edition}^3 Mult{} {C:uncommon}#6# in #7# chance{}',
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return {vars = {card.ability.extra.ANTI, card.ability.extra.eternal, card.ability.extra.ignore, card.ability.extra.invert, card.ability.extra.var1, new_numerator, new_denominator}}
    end,

    calculate = function(self, card, context)
        if context.cardarea == G.jokers and context.joker_main  then
            if (G.GAME.pool_flags.cokelatr_virus or false) then
                return {
                    e_mult = card.ability.extra.emult
                ,
                    func = function()
                        if SMODS.pseudorandom_probability(card, 'group_0_44ffd140', 1, card.ability.extra.odds, 'j_cokelatr_virus', false) then
            ...
            elseif not ((G.GAME.pool_flags.cokelatr_virus or false)) then
                return {
        ...
        if context.buying_card and context.card.config.center.key == self.key and context.cardarea == G.jokers  and not context.blueprint then
                G.GAME.pool_flags.cokelatr_ANTI = true
        end
        if context.selling_self  and not context.blueprint then
                G.GAME.pool_flags.cokelatr_ANTI = not (G.GAME.pool_flags.cokelatr_ANTI or false)
        end
    end
}